...
function getHTML() {
    ...
    html += 'var out = \'\';';
    // Header with Stack Colors button
    html += 'out += \'<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:1rem;padding:0 0.25rem;flex-wrap:wrap;gap:0.5rem">\';';
    html += 'out += \'<h3 style="margin:0;color:#1e3a5f">🎨 By Style <span style="font-size:0.75rem;color:#86868b;font-weight:normal">(\' + items.length + \' styles)</span></h3>\';';
    html += 'out += \'<button class="group-toggle-btn\' + (state.stackByStyle ? " active" : "") + \'" onclick="toggleStackByStylePO()">🎨 Stack Colors</button>\';';
    html += 'out += \'</div>\';';
    html += 'if (state.stackByStyle) {';
    // Stacked mode - group by base style
    html += 'out += \'<div class="dashboard-grid">\';';
    html += 'var styleGroups = {};';
    html += 'items.forEach(function(item) {';
    html += 'var baseStyle = item.style_number.split("-")[0];';
    ...
    html += 'out += \'</div>\';';
    html += '} else {';
    // Original non-stacked product-grid view
    html += 'out += \'<div class="product-grid">\';';
    html += 'items.forEach(function(item) {';
    html += 'var imgSrc = item.image_url || "";';
    html += 'if (imgSrc) { var match = imgSrc.match(/\\/download\\/([a-zA-Z0-9]+)/); if (match) imgSrc = "/api/image/" + match[1]; }';
    html += 'var fileId = imgSrc.startsWith("/api/image/") ? imgSrc.replace("/api/image/", "") : "";';
    html += 'var poCount = item.po_count || (item.pos ? item.pos.length : 1);';
    html += 'out += \'<div class="style-card"><div class="style-image">\';';
    html += 'if (item.commodity) out += \'<span class="commodity-badge">\' + escapeHtml(item.commodity) + \'</span>\';';
    html += 'out += \'<span class="style-badge">\' + poCount + \' PO\' + (poCount > 1 ? "s" : "") + \'</span>\';';
    ...
    html += 'out += \'<div class="style-stats"><div class="style-stat"><div class="style-stat-value">\' + formatNumber(item.total_qty || 0) + \'</div><div class="style-stat-label">Units</div></div>\';';
    html += 'out += \'<div class="style-stat"><div class="style-stat-value money">\' + formatNumber(Math.round(item.total_dollars || 0)) + \'</div><div class="style-stat-label">Value</div></div></div></div></div>\'; });';
    html += 'out += \'</div>\';';
    html += '}';
    html += 'container.innerHTML = out; }';
